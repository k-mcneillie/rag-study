...
def _require_directory(model_path: Path) -> None:
    """Check that a model directory exists before anything tries to load it.

    Args:
        model_path: Directory that should hold the model's files.

    Raises:
        ModelUnavailableError: If the directory is missing.
    """
...
def load_cross_encoder(model_path: Path) -> CrossEncoder:
    """Load a cross-encoder model from a local directory.

    A cross-encoder reads a query and a passage together and scores their
    relevance directly, rather than comparing two independently computed
    vectors. That is what makes it a better judge than the ranker, and also
    why it is too slow to run over a whole corpus: it is applied to a
    shortlist, never to the store.

    The same rules apply as for any other model asset: the directory must
    exist, nothing is fetched, and code shipped with the weights is not run.
    ...
    try:
        ...
        raise ModelUnavailableError(
            f"The cross-encoder at {model_path} could not be loaded."
        ) from exc
    return model
